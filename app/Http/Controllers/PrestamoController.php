...
class PrestamoController extends Controller
{
    ...
    public function index():JsonResponse
    {
        ...
        ($prestamo = Prestamo::where('usuariosede_id', $usuarioAut->id)
    ...
    public function store(Request $request): JsonResponse
{
    ...
    $validarExistencia = Prestamo::where('usuario_id', $usuario->id)
        ->where('estado_prestamo', 'activo')
        ->exists();
    ...
    $prestamoData = $request->except(['codigo_herramienta', 'cantidad']);
    $prestamoData['usuariosede_id'] = $usuariosede_id;
    $prestamoData['usuario_id'] = $usuario->id;
    $prestamoData['codigo_herramienta'] = json_encode($request->codigo_herramienta);
    $prestamoData['cantidad'] = json_encode(array_map('intval', $request->cantidad));
    ...
    Historial::create([
        ...
        'usuariosede_id' => $prestamoData['usuariosede_id'],
        'estado' => 'activo',
    ]);

    if ($request->has('codigo_herramienta') && is_array($request->codigo_herramienta) && is_array($request->cantidad)) {
        ...
        return response()->json(['prestamo realizado' => false, 'message' => 'Formato incorrecto de codigo_herramienta'], 400);
    }
